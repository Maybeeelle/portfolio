...
// Define types for Bubble and animation
interface Bubble {
  x: number;
  y: number;
  size: number;
...
export default function Bubbles() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const updatedBubbles = bubbles.map(bubble => {
        ...
        return bubble;
      }).filter((bubble): bubble is Bubble => bubble !== null); // Remove any null bubbles
  ...
  const createBubblesAtPosition = (x: number, y: number, color?: string) => {
    if (isPaused) {
      setIsPaused(false);
    }
